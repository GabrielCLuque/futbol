                ...
                <div class="flex items-center justify-center h-24 rounded bg-gray-50 dark:bg-gray-800">
                    <div class="text-center">
                        <p class="text-2xl text-gray-400 dark:text-gray-500">
                            ...
                            {{ $partido->jugado ? ' - Resultado: ' . $partido->resultado : ' - No Jugado' }}
                        </p>
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            {{ $partido->fecha->format('d M Y, H:i') }}
                        </p>
                        <a href="{{ route('partidos.edit', $partido->id) }}" class="mt-2 inline-block px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-700">Editar</a>
                    </div>
                </div>
            @endforeach
        </div>
