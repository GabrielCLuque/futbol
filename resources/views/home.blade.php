...
<head>
    ...
    <style>
        .page-bg {
            ...
            background-image: url('table-bg.jfif'); /*otra foto por si hay demasiados partidos con resultado, aunque si se administra bien la paginano deberia llegar a verse */
...
<aside id="default-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 sidebar-bg" aria-label="Sidebar">
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 bg-opacity-70 dark:bg-gray-800 dark:bg-opacity-70">
        <ul class="space-y-2 font-medium">
            ...
            @guest
            <li>
                <a href="{{ route('login') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white group hover-effect">
            ...
            @auth
            <li>
                <a href="{{ route('dashboard.edit', ['id' => Auth::user()->id]) }}" class="btn btn-info flex items-center p-2 text-gray-900 rounded-lg dark:text-white group hover-effect">
            ...
            <li>
                @if(Auth::check() && Auth::user()->admin_status == 1)
                    <a href="{{ route('dashboard.editall') }}" class="btn btn-info flex items-center p-2 text-gray-900 rounded-lg dark:text-white group hover-effect">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5" width="24">
            ...
            <li>
                <form action="{{ route('logout') }}" method="POST" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white group hover-effect">
                    @csrf
            ...
            <li>
                @if(Auth::check() && Auth::user()->admin_status == 1)
                    <a href="{{ route('partidos.create') }}" class="btn btn-info flex items-center p-2 text-gray-900 rounded-lg dark:text-white group hover-effect">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5" width="24">
            ...
            <li>
                @if(Auth::check() && Auth::user()->admin_status == 1)
                    <a href="{{ route('partidos.all') }}" class="btn btn-info flex items-center p-2 text-gray-900 rounded-lg dark:text-white group hover-effect">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5" width="24">
